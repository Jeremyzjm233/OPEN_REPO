resource "aws_security_group" "application_load_balancer" {
  name        = "${var.project_name}-${var.environment}-alb-web-sg"
...
resource "aws_security_group" "web_server_ecs_internal" {
  name        = "${var.project_name}-${var.environment}-web-server-ecs-internal-sg"
  description = "Allow all inbound traffic"
  vpc_id      = data.aws_vpc.vpc.id
...
resource "aws_ecs_task_definition" "web_server" {
  family = "${var.project_name}-${var.environment}-web-server"
...
[
  {
    ...
    "environment": [
      ...
      },
      {
        ...
        "value": "https://airflow.dataservices.aws${var.environment}.internal"
      },
      {
